...
interface StaffAttendanceActionsProps {
  isFinalized: boolean;
...
const StaffAttendanceActions = ({
  ...
  isFinalizing = false,
  isReopening = false,
}: StaffAttendanceActionsProps) => {
  if (isFinalized) {
    return (
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2 text-sm text-text-secondary">
          <Lock className="h-4 w-4" />
          <span>✅ Finalized by Admin - Payroll data locked</span>
        </div>
        {canReopen && (
          <Button
            variant="outline"
...
export default StaffAttendanceActions;
